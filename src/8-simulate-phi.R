...
od <- seq(91, 212)
tssr_values <- seq(-2, 6)
tssr <- rep(tssr_values, each = length(od))
...
foreach (i = 1:max(rem$model), .packages = 'MASS') %dopar%
  {
    ...
    sim_data <- data.frame(Species = rep(rem_reduced$Species,
                           ...
                           OD = rep(od, length(tssr_values) * length(rem_reduced$Species)))
    ...
    for (sp in rem_reduced$Species)
    {
      ...
      tssr_sp_median <- median(rem_species_summary[[sp]]$TSSR)
      design$TSSR <- (design$TSSR - tssr_sp_median) / 24
      design$TSSR2 <- design$TSSR ^ 2
      
      od_sp_median <- median(rem_species_summary[[sp]]$OD)
